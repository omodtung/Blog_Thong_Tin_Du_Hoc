...
// load bài ở tin tức sự kiện



post_articles_5('.main-content-text', 4);

function post_articles_5(container, numberOfPost) {
  let containerPosts = document.querySelector(container);
  let posts = window.localStorage.getItem('post');
  let postArr = JSON.parse(posts);
  let sizeArr = postArr.length;
          ...
          fetch(pathText)
            ...
            .then(content => {
              ...
              paragraphs.forEach((item) => {
                ...
                else if (item.startsWith('Heading') == false && item.startsWith('Picture') == false) {
                  if (index < maxarrayParagrap1) {
                    arrayParagrap1.push(item + ' ');
                    arrayParagrap2.push(item + ' ');
                    index++;
                  } else {
                    arrayParagrap2.push(item + ' ');
                  }
              ...
              structurePost(post.id, `.content-text-${i}`, valueHeading, pictureURL, date, view, arrayParagrap1.join(''), arrayParagrap2.join(''));
...
// cau truc bai tin tuc su kien
function structurePost(postID, containerNumber, headingString, pictureURL, dateString, viewString, paragraph_1String, paragraph_2String) {

  // set gia tri anh
  let picture = document.querySelector(`${containerNumber} .text-img a img`);
  picture.setAttribute('src', pictureURL);
  let aPicture = document.querySelector(`${containerNumber} .text-img a`);
  aPicture.addEventListener('click', (event) => {
    setStatePost(event, postID);
  });

  // set tieu de
  let heading = document.querySelector(`${containerNumber} .text-main .text-title a`);
  heading.innerHTML = headingString;
  heading.addEventListener('click', (event) => {
    setStatePost(event, postID);
  });

  let headingString_inDetail = document.querySelector(`${containerNumber} .modal .modal-title`);
  headingString_inDetail.innerHTML = headingString;

  // set date and view
  let date = document.querySelector(`${containerNumber} .time_view .time_post span`);
  date.innerHTML = dateString;
  let view = document.querySelector(`${containerNumber} .time_view .view span`);
  view.innerHTML = viewString;

  // set doan van gioi thieu ben ngoai
  let paragraph_1 = document.querySelector(`${containerNumber} .text-p p`);
  paragraph_1.innerHTML = paragraph_1String;

  // set doan van trong xem chi tiet
  let paragraph_2 = document.querySelector(`${containerNumber} .modal .modal-dialog .modal-body p`);
  paragraph_2.innerHTML = paragraph_2String;

  // button xem chi tiet
  let buttonDetail = document.querySelector(`${containerNumber} .modal .modal-footer button:nth-child(2)`);
