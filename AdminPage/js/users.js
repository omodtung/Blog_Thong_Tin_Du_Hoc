...
function showDataTable(arrComent) {
    let tableData = document.querySelector(
        ".user-management-page .total-user table"
    );
    tableData.innerHTML = "";
...
function deleteUser(idUser) {
    let decision = window.confirm("Bạn có chắc muốn xóa tài khoản này");
    if (decision == true) {
        let user = window.localStorage.getItem("user");
        let users = JSON.parse(user);
        let usersAterDelete = [];
...
function filter() {
    let condition = document.querySelector(
        ".user-management-page .action-filter-user input"
    ).value;
    ...
    users.forEach((value) => {
        if (
            value.idUser.toLowerCase().includes(condition.toLowerCase()) ||
            value.fullName.toLowerCase().includes(condition.toLowerCase()) ||
            value.Password.toLowerCase().includes(condition.toLowerCase()) ||
            value.role.toLowerCase().includes(condition.toLowerCase()) ||
            value.state.toLowerCase().includes(condition.toLowerCase())
        ) {
